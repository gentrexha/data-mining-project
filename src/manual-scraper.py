...
def _extract_html(bs_data):
    ...
    for item in k:
        ...
        # Time created
        postTimes = item.find_all(class_=re.compile('_5ptz*'))
        postDict['Time'] = ""
        for postTime in postTimes:
            postDict['Time'] = postTime.get('title')

        # Number of Shares
        postShares = item.find_all(class_='_3rwx _42ft')
        postDict['Shares'] = 0
        for postShare in postShares:
            # print(postShare.text.split()[1])
            postDict['Shares'] = float(postShare.text.split()[1])

        # Number of comments
        postComments = item.find_all(class_='_3hg- _42ft')
        postDict['Comments'] = 0
        for postComment in postComments:
            postDict['Comments'] = float(postComment.text.split()[1])
